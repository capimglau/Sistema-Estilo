...
grupo("Agenda do Dashboard — painel mostra 3 dias");
{
  const dia = (iso, n) => ({ isoD: iso, evsDia: Array.from({ length: n }, (_, i) => ({ id: iso + "-" + i })) });

  eq("o painel mostra 3 dias", F.AGK2_LIM_DIAS, 3);

  // Mais dias que o limite: renderiza 3, o resto vira "+N".
  {
    const c = F.agk2CorteDias([dia("d1", 2), dia("d2", 4), dia("d3", 1), dia("d4", 5), dia("d5", 3)], 3);
    eq("renderiza exatamente 3 dias", c.diasMostrar.length, 3);
    eq("o 3º dia é o último visível", c.diasMostrar[2].isoD, "d3");
    // d4 (5) + d5 (3) — o "+N" conta os dias inteiros que ficaram de fora
    eq("os ocultos somam os dias que ficaram de fora", c.ocultosLista.length, 8);
  }

  // Um dia cheio NÃO pode consumir a cota dos outros — foi por isso que o
  // corte deixou de ser por cartão e passou a ser por dia.
  {
    const c = F.agk2CorteDias([dia("d1", 9), dia("d2", 1), dia("d3", 1)], 3);
    eq("dia cheio não engole os outros dois", c.diasMostrar.length, 3);
    eq("e nada fica oculto por conta dele", c.ocultosLista.length, 0);
  }

  // Menos dias que o limite: mostra tudo, sem baralho global.
  {
    const c = F.agk2CorteDias([dia("d1", 2), dia("d2", 1)], 3);
    eq("cabe tudo, mostra tudo", c.diasMostrar.length, 2);
    eq("sem nada oculto", c.ocultosLista.length, 0);
  }

  // Exatamente no limite: nada oculto.
  eq("3 dias exatos não escondem nada",
    F.agk2CorteDias([dia("d1", 1), dia("d2", 1), dia("d3", 1)], 3).ocultosLista.length, 0);

  // Agenda expandida / desktop / modal: sem corte.
  {
    const c = F.agk2CorteDias(null, 3);
